                ...
                $.get(core.getUrl('diypage/getInfo'),{url:url,num:num,paramsType:'goods'},function(ret){
                    ...
                    if(ret.status == 1){
                        ...
                        $.each(list,function(id,item){
                            ...
                            html += '<p class="productprice noheight"></p>';
                            //李小酷：增加会员折扣
                            html+=  show_member_price?'<div class="li_price_block left-0"><span class=" text-danger" style="color:#c8952a"> ¥ '+ (item.minprice/10*member_discount).toFixed(2) +'</span><span class="member-price">会员价 <span></span></span></div>':'';
                            html += '<div class="price">';
                            html += '<span class="text" style="color: #ff5555;">';
                            html += '<p class="minprice">¥'+ item.minprice +'</p>';
                            html += '</span>';
