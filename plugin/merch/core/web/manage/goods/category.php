...
//				}
		...
		$children = array();
                if($_W['shopset']['shop']["cateformerch"]==0){
                   	$category = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_category') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY merchid ASC, parentid ASC, displayorder DESC'));
 
                }else{
                    	$category = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_category') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' and merchid = \'' . $_W['merchid'] . '\' ORDER BY merchid ASC, parentid ASC, displayorder DESC'));
	...
	public function enabled()
	{
        ...
        //create by Culbert on April 4 2020 
        public function add()
        {
            $this->post();
        }
