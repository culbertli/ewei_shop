...
class ListController extends PluginMobilePage
{
	public function main()
	{
		...
		if (isset($_GPC['group_id'])) {
			...
			if ($goodsGroup['goods_type'] == 0) {
                ...
                if($_W["shopset"]["shop"]["cateformerch"]==0){
                    ...
                    $cateinfo = pdo_get('ewei_shop_category', array('id' => $_GPC['cate'], 'uniacid' => $_W['uniacid'], 'enabled' => 1,'merchid'=>isset($_W["merchid"])?$_W["merchid"]:0));
                }
		...
		if ($cateinfo) {
			...
			$category['downcate'] = pdo_getall('ewei_shop_category', array('level' => 1, 'uniacid' => $_W['uniacid'], 'enabled' => 1), array() , '' , 'displayorder DESC');
                        //增加多商户分类的获取限制
                        if($_W["shopset"]["shop"]["cateformerch"]==0){
                            $category['downcate'] = pdo_getall('ewei_shop_category', array('level' => 1, 'uniacid' => $_W['uniacid'], 'enabled' => 1), array() , '' , 'displayorder DESC');
                        }else{
                            $category['downcate'] = pdo_getall('ewei_shop_category', array('level' => 1, 'uniacid' => $_W['uniacid'], 'enabled' => 1,'merchid'=>isset($_W["merchid"])?$_W["merchid"]:0), array() , '' , 'displayorder DESC');
                         }
                }
