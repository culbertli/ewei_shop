...
class PcModel extends PluginModel
{
	...
	public function getAllCategories()
	{
		global $_W;
                //李小酷：加入多商户商品分类限制条件
                if($_W["shopset"]["shop"]["cateformerch"]==0){
                    $data = pdo_getall('ewei_shop_category', array('uniacid' => $_W['uniacid'], 'enabled' => 1), array('id', 'parentid', 'name', 'level'), '', 'displayorder desc');
                }else{
                    $data = pdo_getall('ewei_shop_category', array('uniacid' => $_W['uniacid'], 'enabled' => 1,'merchid'=>isset($_W["merchid"])?$_W["merchid"]:0), array('id', 'parentid', 'name', 'level'), '', 'displayorder desc');
                }
	...
	public function fetchGoodsByType($id, $type)
	{
		...
		case static::GOODS_CATE:
			$cateId = $id;
                        //添加商品父类查询，判断数据是否为空
                        //获取子类id
                        $childrenIds=pdo_fetchall('select id from ' . tablename('ewei_shop_category') . (' where uniacid = :uniacid AND enabled = :enabled  AND parentid = :parentid'), array(':uniacid' => $_W['uniacid'],':enabled'=>1,':parentid'=>$cateId ));
                        if($childrenIds){
                            $childrenIdStr = '(';
                            foreach($childrenIds as $ck => $cv ){$childrenIdStr.=$cv['id'].',';}
                            $childrenIdStr = $childrenIdStr.$cateId.')';
                            $goodsIds = pdo_fetchall('select id from ' . tablename('ewei_shop_goods') . (' where uniacid = :uniacid AND deleted = :deleted  AND (pcate = :pcate or cates in '.$childrenIdStr.' )'), array(':uniacid' => $_W['uniacid'],':deleted'=>0,':pcate'=>$cateId ));
                        }
//			$goodsIds = pdo_fetchall('select id from ' . tablename('ewei_shop_goods') . (' where uniacid = :uniacid AND deleted = :deleted  AND (pcate = :pcate )'), array(':uniacid' => $_W['uniacid'],':deleted'=>0,':pcate'=>$cateId ));
                        if(!$goodsIds){
                            //李小酷1，添加查询条件 deleted=0
                            $goodsIds = pdo_fetchall('select id from ' . tablename('ewei_shop_goods') . (' where uniacid = :uniacid AND deleted = :deleted  AND FIND_IN_SET(' . $cateId . ', cates) '), array(':uniacid' => $_W['uniacid'],':deleted'=>0));
	...
	{
		$allKeys = array('thumb', 'import_image', 'bottom_image', 'top_image', 'img', 'bottom_adv_image');//删除了'import_url'
