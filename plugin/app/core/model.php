...
public function getRelease($authid,$uri='')
    ...
    global $_W;
        ...
        public function sendSubscribeMessage($openid, $msgdata, $type)
        {
            ...
            $tmp = array();
            if ($type == "pay") {
                $tmp = array("character_string1" => array("value" => $msgdata["ordersn"]), "thing3" => array("value" => mb_substr($msgdata["title"], 0, 20)), "amount4" => array("value" => "￥" . $msgdata["price"] . "元"), "date2" => array("value" => $msgdata["time"]));
            } else {
                if ($type == "send") {
                    $tmp = array("character_string1" => array("value" => $msgdata["ordersn"]), "thing2" => array("value" => mb_substr($msgdata["title"], 0, 20)), "phrase3" => array("value" => mb_substr($msgdata["express"], 0, 5)), "character_string4" => array("value" => $msgdata["expresssn"]), "date5" => array("value" => $msgdata["time"]));
                } else {
                    if ($type == "autosend") {
                        $tmp = array("character_string1" => array("value" => $msgdata["ordersn"]), "thing2" => array("value" => mb_substr($msgdata["title"], 0, 20)), "date5" => array("value" => $msgdata["time"]), "thing8" => array("value" => mb_substr($msgdata["remark"], 0, 20)));
                    } else {
                        if ($type == "receive") {
                            $tmp = array("character_string1" => array("value" => $msgdata["ordersn"]), "thing2" => array("value" => mb_substr($msgdata["title"], 0, 20)), "time3" => array("value" => $msgdata["time"]));
                        }
                    }
                }
            }
