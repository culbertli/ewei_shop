...
class Rank_EweiShopV2Page extends CommissionMobileLoginPage
{
	...
	public function main()
	{
		...
		case '0':
			...
			//李小酷：修复累计佣金排名展示1
                        $users = pdo_fetchall('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('ewei_shop_member') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']),'','commission_total ASC');
                        ...
                        foreach($users as $uk => $uv){
                            if($uv['id']==$user['id']){$user['paiming']=$uk+1;break;}
                        }
