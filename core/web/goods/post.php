...
if($_W['shopset']['shop']["cateformerch"]==0){
    ...
    $category =m('shop')->getSelfCategory(true, true);
}
/**李小酷3 控制商品分类输出**/
...
if ($_W['ispost']) {
	...
	if (empty($id)) {
		$data['merchid'] = 0;
                ...
                /*李小酷1：新插入商品信息时，处理商品详情*/
                $data_deal_arr=m('culbert')->getDiyPageByGood(0,intval($data['diypage']),$data['content']);
                $data['content']=$data_deal_arr[0];
                $data['usediypage']=$data_deal_arr[1];
                //李小酷1.1：插入商品时，看看是否有自定义【全部商品】分类id
                if(!empty($_GPC['li_all_goods_id'])&&!strstr($data['cates'],$_GPC['li_all_goods_id'])){
                    $data['cates']=$data['cates'].','.$_GPC['li_all_goods_id'];
                }
		...
		$old_data = pdo_fetch('SELECT nocommission,marketprice,diypage FROM ' . tablename('ewei_shop_goods') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		/*李小酷2：编辑商品信息时，处理商品详情*/
                $data_deal_arr=m('culbert')->getDiyPageByGood($id,intval($data['diypage']),$data['content']);
                $data['content']=$data_deal_arr[0];
                $data['usediypage']=$data_deal_arr[1];
