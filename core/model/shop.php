...
class Shop_EweiShopV2Model
{
		...
		if (empty($allcategory) || $refresh) {
			...
			$children = array();
                        ...
                        if($_W['shopset']['shop']["cateformerch"]==0){
                            ...
                            $category = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_category') . ' WHERE uniacid =:uniacid AND enabled=1 AND merchid=:merchid ORDER BY merchid ASC,parentid ASC, displayorder DESC', array(':uniacid' => $_W['uniacid'],':merchid'=>$_W["merchid"]?$_W["merchid"]:0));
                        }
	...
	public function getFullCategory($fullname = false, $enabled = false)
	{
        ...
        public function getSelfCategory($fullname = false, $enabled = false)
	{
		global $_W;
		$allcategorynames = m('cache')->getArray('allcategorynames');
		$shopset = m('common')->getSysset('shop');
		$allcategory = array();
		$sql = 'SELECT * FROM ' . tablename('ewei_shop_category') . ' WHERE uniacid=:uniacid and merchid=:merchid';

		if ($enabled) {
			$sql .= ' AND enabled=1';
		}
		$sql .= ' ORDER BY parentid ASC, displayorder DESC';
		$category = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'],':merchid'=>$_W["merchid"]?$_W["merchid"]:0));
		$category = set_medias($category, array('thumb', 'advimg'));
