	...
	public function getList($args = array())
	{
		...
		array_walk($list, function(&$goods) {
			if ($goods['ispresell'] == 1 && $goods['presellprice'] < $goods['minprice']) {
                ...
                /*李小酷:如设置显示列表中的会员价*/
                if(m('culbert')->show_member_price){//假如有开启标识
                    //按照给定的折扣
                    $member_discount=m('culbert')->member_discount!==0?m('culbert')->member_discount:m('culbert')->getCurrentMemberDiscount();
                    foreach($list as $list_k => $list_v){
                        $list[$list_k]['li_memberprice']=$list[$list_k]['minprice']/10*$member_discount;
                    }
                }
