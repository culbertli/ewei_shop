	...
	public function lazy($html = '')
	{
		...
		$html = preg_replace_callback('/<img.*?src=[\\\\\'| \\"](.*?(?:[\\.gif|\\.jpg|\\.png|\\.jpeg]?))[\\\\\'|\\"].*?[\\/]?>/', function($matches) use($_W) {
			...
			if (strexists($images, 'http://') || strexists($images, 'https://')) {
				$ifUseMap=strpos($images, 'usemap');//判断是否实用了热区
				if($ifUseMap){
                                        return preg_replace('/usemap/', 'liusemap', $images);//干预热区，是其失效
					//return $images;//取消懒加载
				}else{
					return preg_replace('/src=/', 'src="/attachment/images/global/loading.gif" data-lazy=', $images);//懒加载
				}
